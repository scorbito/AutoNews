...
_SYSTEM = (
    ...
    "  2) '첨부 문서 본문(추출)'을 최우선으로 보라. 거기에 실제 기사로 쓸 내용"
    "(하나의 사건·행사·정책을 문장으로 설명한 보도자료 본문)이 있으면 → skip=false.\n"
    "  3) 그러나 첨부/본문이 '보도자료 배포계획'·'주간(행사)일정표'·목차·색인·수신안내처럼 "
    "앞으로 배포할 제목·날짜·담당부서를 표/목록으로 나열만 한 메타문서면 → 반드시 skip=true(보류). "
    "hwp 등 문서가 첨부돼 있어도, 그 내용이 문장 본문이 아니라 제목·일정 나열이면 skip=true. "
    "예: 제목이 '…배포계획', '주간행사계획', '주간일정'이고 본문이 표(제목·부서·담당자·날짜) 나열이면 skip=true.\n"
